...
var eventChan = make(chan SubscriptionData)
	...
	priceQuery = `subscription {
		Solana {
		  DEXTradeByTokens(
			...
			"sQB8ng2BgciryiHNvwUyFZVgLYdYM1XnQsnKg96pump", "DPxY4KD723wsLwsiAGgjznXCMxDfUbjruxXeFLsspump", "CDCAVw72kXzW9X57sn9Knmh1ttX7uZPahrdhSePqpump", "Gr4t1Aw1qUFrWLj2wgKrYvQ66dT7m2iZ6EWnTi4pump"]}}}, Transaction: {Fee: {}}}
		  ) {
			Trade {
			  Amount
			  PriceInUSD
			...
			if eventMsg.Type == "data" {
				for _, tradeData := range eventMsg.Payload.Data.Solana.DEXTradeByTokens {
					if tradeData.Trade.Currency.Symbol == "TrumpOnX" {
						...
						fmt.Printf("Received and processed message: %v\n", eventMsg)
					}
				}
			}
